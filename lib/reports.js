...
/**
 * Dapatkan rentang siklus bulanan berdasarkan MONTHLY_START_DATE.
 * Contoh: start=25 → 25 bulan ini s/d 24 bulan depan.
 *         start=1  → 1 bulan ini s/d akhir bulan ini.
 *         start=28 → 28 bulan ini s/d 27 bulan depan.
 *
...
function getMonthRange() {
  ...
  const start = new Date(Date.UTC(startYear, startMonth, MONTHLY_START_DATE, 7, 0, 0));

  // End = MONTHLY_START_DATE - 1 di bulan berikutnya.
  // Date.UTC dengan tanggal 0 menghasilkan hari terakhir bulan sebelumnya,
  // sehingga end selalu tepat 1 hari sebelum start bulan depan.
  const end = new Date(Date.UTC(startYear, startMonth + 1, MONTHLY_START_DATE - 1, 23, 59, 59, 999));
...
/**
 * Filter transaksi bulan ini (MONTHLY_START_DATE s/d 24 bulan depan, pengeluaran saja).
 */
 ...
 * Misal: bulan ini 25 Jun - 24 Jul → bulan lalu 25 Mei - 24 Jun
 */
