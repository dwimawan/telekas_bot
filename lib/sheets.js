...
/**
 * Parse nominal dari string yang mungkin berformat rupiah ("Rp 7.000") atau
 * plain number ("7000"). Hapus semua karakter non-digit kecuali tanda minus.
 */
function parseNominalSafe(val) {
  if (val === undefined || val === null || val === "") return 0;
  const str = String(val);
  // Hapus semua karakter selain digit dan minus
...
async function getAllTransactions() {
  ...
  return rows.slice(1).map((row) => ({
    ...
    nominal: parseNominalSafe(row[3]),
