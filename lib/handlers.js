...
async function handleBulanan(chatId) {
  try {
    ...
    const today = nowWIB();
    const diffDays = Math.max(1, Math.ceil((today.getTime() - monthStart.getTime()) / (1000 * 60 * 60 * 24)) + 1);
    ...
    if (monthlyInsight) {
      const lastLabel = `${formatDate(new Date(monthStart.getTime() - 30 * 24 * 60 * 60 * 1000))} - ${formatDate(new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000))}`;
      insight = {
        avgLabel: `Bulan lalu`,
    ...
    const label = `${formatDate(monthStart)} - ${formatDate(today)}`;
